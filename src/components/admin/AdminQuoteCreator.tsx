...
export default function AdminQuoteCreator({ onClose, onCreated, prefillItems, prefillCustomer, editQuoteId, editDefaults }: AdminQuoteCreatorProps) {
    ...
    const updateItemQty = (index: number, raw: string) => {
        const newItems = [...items]
        if (raw === '' || raw === '-') {
            newItems[index].quantity = 0
        } else {
            const qty = parseInt(raw)
            if (!isNaN(qty)) newItems[index].quantity = qty
        }
        setItems(newItems)
    }
                                            ...
                                            {items.map((item, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td className="px-4 py-2 text-center">
                                                        <input type="number" value={item.quantity} onChange={e => updateItemQty(i, e.target.value)}
