...
function RegisterContent() {
    ...
    if (type === 'b2c') {
        return (
            <div>
                <h1 className="text-3xl font-bold text-center mb-2">{t('createCustomer')}</h1>
                <p className="text-center text-gray-500 mb-6">{t('forHomeowners')}</p>
                <B2CRegistrationForm />
        ...
        <div className="max-w-4xl mx-auto py-12 px-4">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <Link href="/auth/register?type=b2b" className="block p-8 bg-white rounded-xl shadow-md border hover:border-blue-500 hover:shadow-lg transition-all text-center group">
                    ...
                    <h2 className="text-xl font-bold mb-2">{t('businessAccount')}</h2>
                    <p className="text-gray-500">{t('businessDesc')}</p>
                </Link>
            </div>
        </div>
    )
